...
def test_stage7n_postgres_exposes_ml_and_governed_decisions() -> None:
    ...
    assert "decision_ops.decision_case" in sql
    assert "decision_ops.decision_evidence" in sql
    assert "decision_ops.replenishment_draft" in sql
...
def test_stage7n_decision_case_does_not_chain_security_invoker_view() -> None:
    sql = Path("infra/docker/postgres/sql/012_stage7n_powerbi_readonly.sql").read_text(
        encoding="utf-8"
    )
    decision_sql = sql.split("CREATE OR REPLACE VIEW bi.v_decision_case", 1)[1].split(
        "CREATE OR REPLACE VIEW bi.v_decision_audit", 1
    )[0]
    assert "WITH (security_invoker = false)" in decision_sql
    assert "decision_ops.v_case_workbench" not in decision_sql
    assert "FROM decision_ops.decision_case c" in decision_sql
    assert "decision_ops.decision_evidence" in decision_sql
    assert "decision_ops.replenishment_draft" in decision_sql


def test_stage7n_runner_executes_bi_role_runtime_read_smoke() -> None:
    runner = Path("scripts/run_stage7n_powerbi.py").read_text(encoding="utf-8")
    assert "SET ROLE pharmstock_bi" in runner
    assert "SELECT * FROM bi.{view_name} LIMIT 0" in runner
    assert '"runtime_readable_views": runtime_readable' in runner
...
def test_stage7n_all_bi_views_are_explicit_security_definer() -> None:
    sql_path = Path("infra/docker/postgres/sql/012_stage7n_powerbi_readonly.sql")
    sql = sql_path.read_text(encoding="utf-8")
    for view_name in POSTGRES_VIEWS:
        block = sql.split(f"CREATE OR REPLACE VIEW bi.{view_name}", 1)[1]
        block = block.split("AS", 1)[0]
        assert "security_invoker = false" in block


def test_stage7n_runtime_smoke_uses_psql_exit_status_not_pass_text() -> None:
    runner = Path("scripts/run_stage7n_powerbi.py").read_text(encoding="utf-8")
    assert '"ON_ERROR_STOP=1"' in runner
    assert "except subprocess.CalledProcessError as exc" in runner
    assert "smoke.splitlines()" not in runner
    assert "SELECT 'PASS'" not in runner
